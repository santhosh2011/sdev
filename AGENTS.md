...
- **Install-time deps**: `install` hard-requires bash ≥ 4, mikefarah `yq` v4, and **git** (worktrees); **docker is runtime-only** (needed for `sdev up`) so it's a *warning*, not a fatal check. This is deliberate — GitHub's `macos-latest` runners have no Docker, so a fatal docker gate would break the bats matrix (`install.bats` + `self_update.bats` both run the real installer). Covered by the "install treats docker as runtime-only" test. (Legacy `bootstrap`'s `check_deps` still treats docker as fatal — it's the in-repo dev path, not the shipped installer.)
